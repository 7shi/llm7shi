...
**Solution**: `generate_with_schema()` maps `include_thoughts=False` to `extra_body={"reasoning": {"enabled": False}}` and hands it to the OpenAI delegate via the `extra_body` argument. `enabled: False` fully disables the reasoning process; `exclude: True` (a separate flag) only hides reasoning tokens from the response while the model still thinks. The mapping is scoped to the `openrouter:` prefix (the only vendor here that supports it). With the default `include_thoughts=True`, no `extra_body` is built. Capturing and displaying any reasoning that OpenRouter returns is handled downstream in `openai.py`.
