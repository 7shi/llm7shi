...
### Italic vs Bold vs List Markers
**Problem**: A single `*` is overloaded in markdown: `*italic*` is emphasis, `**bold**` is strong, and a line-leading `* ` (or indented `  * `) is a list bullet. Treating every `*` as an italic toggle would turn bullet lists into stray color runs, and a closing `*italic*` is normally followed by a space (so a naive "followed by a space ⇒ literal" rule would never close it).

**Solution**: `**` is handled first as bold. For a lone `*`, an already-open italic is **always closed** (so the closing delimiter works regardless of the following character); otherwise the `*` only **opens** italic when followed by a non-space. That single rule leaves `* ` and `  * ` list markers literal while still supporting `*italic*` anywhere in a line. Italic uses `ITALIC_ON = Fore.YELLOW`: Colorama has no italic attribute, so yellow stands in for it. In streaming, a lone trailing `*` is buffered until the next chunk disambiguates `**` vs `*` (and reveals the following character); on flush it closes an open italic or, with nothing following, stays literal.
...
Because **markup inside inline code is left literal** (a `` ` `` span's contents are not re-parsed, so `*`/`**` inside it stays a literal asterisk), `code` is always the innermost element. Other formats compose freely through the same stack — e.g. `*italic*` nests inside `**bold**` (`**bold *italic* bold**`), and closing the italic re-applies the bold color. The legacy `bright_mode`/`code_mode` attributes remain available as read-only properties derived from the stack (`"bold" in stack` / `"code" in stack`).
