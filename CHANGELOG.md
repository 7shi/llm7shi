...
### Added
- **`Client.usages`** - Every call's token usage is collected on the client, including responses regenerated by the quality retry, so `sum(client.usages)` gives what a run actually consumed. `show_usage=True` prints each call's usage as it arrives
- **`llm7shi` command** - The command-line entry point is now installed as a console script, so `llm7shi usage show` / `llm7shi usage merge` and `llm7shi md <file>` work directly (`uv run llm7shi ...` inside a project); `llm7shi --version` prints the installed version
